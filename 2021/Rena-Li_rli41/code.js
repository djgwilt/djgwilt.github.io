...
function updatePosition() {
	if (xDir != 0 || yDir != 0) {
		...
		if (handleEnd() == false) { 
			document.getElementById("r" + row + " c" + column).className = characterClass;
		}
	}
...
function handleEnd(){
    if (!document.getElementById("r" + row + " c" + column) || document.getElementById("r" + row + " c" + column).className == "Wall") {
        handleCrash();
        return true;
    } else if (document.getElementById("r" + row + " c" + column).className == "house")
    {
        handleWin();
        return true;
    }
