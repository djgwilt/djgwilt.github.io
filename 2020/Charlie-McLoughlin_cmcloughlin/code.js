...
function runGame() {
	intervalHandle = setInterval(updatePosition, 225)
}
...
function updatePosition() {
	if (xDir != 0 || yDir != 0) {
		...
		if (!document.getElementById("r"+row+" c" + column) || document.getElementById("r"+row+" c" + column).className == "wall") {
			handleCrash()
		} else if (document.getElementById("r"+row+" c" + column).className == "win") {
            handlewin()
        } else if (document.getElementById("r"+row+" c" + column).className == "flags") {
            flags += 1
            if (flags == 5) {
               document.getElementById("r2 c10").className = "fake" 
            }
...
function handlewin() {
    ...
    if (flags == 5) {
        ...
        document.getElementById("Message").innerText = "not enough flags!"
    }
