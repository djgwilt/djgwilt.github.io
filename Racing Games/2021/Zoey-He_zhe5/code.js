...
function updatePosition() {
	if (xDir != 0 || yDir != 0) {
		...
		if (handleEnd()== false) {
			...
			document.getElementById("r"+ row + " c" + column).className = "Koro"
		}
	}
...
function handleEnd()
{
    if (!document.getElementById("r"+ row + " c" + column)|| document.getElementById("r"+ row + " c" + column).className == "dagger") {
        handleCrash();
        return true;
...
function handlewin() {
	window.clearInterval(intervalHandle)
	document.getElementById("Message").innerText = "Good JOB you didn't get assassinated :)) "
}
