...
function updatePosition() {
	if (xDir != 0 || yDir != 0) {
		...
		if (handleEnd() == false) {
            document.getElementById("r" + row + " c" + column).className = GhostClass
		}
    }
}
...
function handleCrash() {
	...
	document.getElementById("Message").innerText = "The castle claims you once more..."
}
...
function handleWin() {
    ...
    if(CoinCount == CoinMax) {
        window.clearInterval(intervalHandle)
        document.getElementById("Message").innerText = "You escape... for now..."
    else 
    }
}
