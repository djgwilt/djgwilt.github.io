...
// to remember jerry class (jerrymouse or jerrymouse2 - flipped or not)
var JerryClass = "Jerry"
...
function updatePosition() {
	if (xDir != 0 || yDir != 0) {
		...
		if (handleEnd() == false) {
			document.getElementById("r" + row + " c" + column).className = JerryClass
		}
	} 
}
...
function handleEnd()
{
    if (!document.getElementById("r" + row + " c" + column))  {
		...
		handleCrash();
        return true; 
    } else if (document.getElementById("r" + row + " c" + column).className == "wallSpike")   {
		handleCrash();
        return true; 
...
function handleWin() {
    JerryCount++
    if(JerryCount == JerryMax){
	window.clearInterval(intervalHandle)
	document.getElementById("Message").innerText = "GREAT JOB! YOU WIN!!"
    }    
}