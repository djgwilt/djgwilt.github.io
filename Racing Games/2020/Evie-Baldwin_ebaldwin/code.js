...
// the function called when a key is pressed - sets direction variable
    ...
    function handleEnd() {
        if(!document.getElementById("r" + row + " c" + column) || document.getElementById("r" + row + " c" + column).className == "Wall") {
            handleCrash();
            return true;
        } else if(document.getElementById("r" + row + " c" + column).className == "Finish") {
            handleWin();
            return true;
        } else if(document.getElementById("r" + row + " c" + column).className == "Token") {
            handleBread();
        }
        return false;
    }
    ...
    function handleBread() {
        count += 1
        document.getElementById("Message").innerText = "Eat me !" + count
    }

    function handleWin() {
        window.clearInterval(intervalHandle)
        document.getElementById("Message").innerText = "You Won!!! :) your score was " + count + "!"
    }